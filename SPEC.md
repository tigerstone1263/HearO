...
- [ ] P1-13 일시정지 UX
  - 구현 피처: 우측 상단에 일시정지 버튼을 띄우고, 누르면 엔진을 멈추고 팝업을 표시하며 팝업에서 홈/계속/설정 액션을 제공
  - 연관 피처: 오버레이 시스템, `HearOGame.pauseGame`/`resumeGame`, 팝업 내 버튼
  - 검증:
    - Start 이후 우측 상단에 Pause 버튼이 나타나며 눌렀을 때 몬스터/타이머 움직임이 멈추고 팝업이 열림
    - 팝업의 “계속하기”는 게임을 다시 재개하고 버튼만 남기며, “홈으로”는 게임을 초기화하고 홈을 보여주고, “설정”은 메시지를 띄움

- [ ] P1-14 숲 타일 무한 맵 (청크 기반)
  - 구현 피처: `assets/tiles/forest/forest.json` 설정에 따라 타일 속성/가중치를 읽어 숲 타일 청크를 생성하고, 플레이어 이동에 따라 무한 맵이 확장되도록 함
  - 연관 피처: 카메라 추적, 맵 렌더링, 성능 최적화(청크 관리)
  - 검증:
    - 플레이어 이동 시 주변 청크가 동적으로 로드/언로드되며 화면에 빈 공간이 나타나지 않음
    - 숲 타일이 `forest.json` 가중치 규칙을 반영하여 자연스러운 변주(이끼 군집 등)를 보여줌
