...
- [ ] P1-02 캐릭터 스프라이트 시트 애니메이션 (Idle/Walk)
  - 구현 피처: 플레이어 스프라이트 로딩, Idle/Walk 애니메이션 전환
  - 연관 피처: 입력 이동 로직, 상태 기반 애니메이션
  - 검증:
    - 이동 입력 시 Walk 애니메이션으로 전환
    - 입력이 없을 때 Idle 애니메이션으로 복귀

- [ ] P1-03 좌측 하단 조이스틱 연동 및 8방향 이동
  - 구현 피처: 가상 조이스틱 입력, 8방향 이동/속도 제어
  - 연관 피처: 카메라 추적, Listening Circle 위치 갱신, 몬스터 추적 AI
  - 검증:
    - 조이스틱 방향에 맞게 캐릭터가 이동
    - 대각선 입력이 8방향으로 자연스럽게 반영

- [ ] P1-04 Listening Circle 렌더링 및 충돌 감지 레이어
  - 구현 피처: 캐릭터 기준 반투명 원 렌더링, 몬스터 진입 감지
...
- [ ] P2-01 오디오 엔진 연동 및 음계별 리소스 확보
  - 구현 피처: audioplayers 등 연동, 음계별 샘플 재생
  - 연관 피처: 몬스터 음계 트리거, 건반 입력 피드백
  - 검증:
    - 지정한 음계가 즉시 재생되고 중복 재생 시 안정적으로 처리

- [ ] P2-02 몬스터 스폰 시스템 (화면 밖 생성 후 캐릭터 추적)
  - 구현 피처: 스폰 타이머, 오프스크린 생성, 추적 이동
  - 연관 피처: 난이도 매니저, 스테이지별 속도 변경
  - 검증:
    - 몬스터가 화면 밖에서 생성되어 캐릭터 방향으로 이동

- [ ] P2-03 몬스터별 고유 음계 할당 및 Listening Circle 진입 재생
...
- [ ] P2-04 몬스터 분노 모드 상태 머신
  - 구현 피처: 오답 판정 시 분노 상태 전환, 색상/속도 변화
  - 연관 피처: 판정 시스템, 몬스터 이동 로직
  - 검증:
    - 오답 입력 시 즉시 분노 상태로 전환되고 속도가 증가
